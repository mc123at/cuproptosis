...
#train.unicox
tcga.surv <- read.table("1.train for each gene cox.txt", row.names = 1, check.names = F, stringsAsFactors = F, header = T)
head(tcga.surv)
...
#valid1.unicox
geo.surv <- read.table("2.valid1 for each gene cox.txt", row.names = 1, check.names = F, stringsAsFactors = F, header = T)
head(geo.surv)
...
#valid2.unicox
geo.surv22 <- read.table("3.valid2 for each gene cox.txt", row.names = 1, check.names = F, stringsAsFactors = F, header = T)
head(geo.surv22)
...
tabletext[16,] <- c("GSE72094",NA,NA,NA,NA) 
tabletext[30,] <- c("GSE68465",NA,NA,NA,NA) 
...
forestplot(labeltext=tabletext,
           ...
           zero=0,#zero线横坐标
           lwd.zero=2,#zero线宽
           xticks = c(-2,-1,0,1,2),#横坐标刻度根据需要可随意设置
           lwd.xaxis=2,
           xlab=expression("log"[2]~"HR"),
           hrzl_lines=list("1" = gpar(lwd=2, col="black"),#第二行顶部加黑实线
                           "2" = gpar(lwd=1, col="grey50", lty=2),#第二行顶部加灰色虚线
                           "16" = gpar(lwd=1, col="grey50", lty=2),#第九行顶部加灰色虚线
                           "30" = gpar(lwd=1, col="grey50", lty=2),#第九行顶部加灰色虚线
                           "44" = gpar(lwd=2, col="black")),#最后一行底部加黑线，""中数字为nrow(tabletext) + 1
