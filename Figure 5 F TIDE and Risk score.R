...
tcga_expr=read.table("tideandriskscore.txt", header=T,sep="\t",row.names=1,check.names=F)
tcga_expr[1:3,1:3]
...
head(cors)
write.table(cors,file="1.allCOR.txt",sep="\t",col.names = NA, row.names=T,quote=F) #??һ??????Ϊ?գ?????NA???Խ???
...
write.table(sortgenesexps,file="2.topGeneExp.txt",sep="\t",col.names = NA, row.names=T,quote=F) #??һ??????Ϊ?գ?????NA???Խ???
