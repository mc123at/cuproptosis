...
topnumber = 15 
pointnumber = 50 
tcga_expr=read.table("sel.immune.LUAD.txt", header=T,sep="\t",row.names=1,check.names=F)
tcga_expr[1:3,1:3]
...
head(cors)
write.table(cors,file="1.allCOR.xls",sep="\t",col.names = NA, row.names=T,quote=F) 
...
write.table(sortgenesexps,file="2.topGeneExp.xls",sep="\t",col.names = NA, row.names=T,quote=F) 
